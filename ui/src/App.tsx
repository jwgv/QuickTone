...
import {analyze, analyzeBatch, health, modelStatus, type SentimentResponse, warmModels,} from "./lib/api";
    ...
    return (
        <div className="inline-flex items-center gap-2">
            ...
            <div className="flex rounded-lg border border-base-border overflow-hidden">
                <button
                    className={`px-3 py-1 text-sm ${theme === "dark" ? "bg-primary-500 text-white" : "bg-base-bg text-slate-300"}`}
                    onClick={() => setTheme("dark")}
                >
                    Dark
                </button>
                <button
                    className={`px-3 py-1 text-sm ${theme === "light" ? "bg-primary-500 text-white" : "bg-base-bg text-slate-300"}`}
...
export default function App() {
    ...
    const [model, setModel] = useState<
        "vader" | "distilbert" | "distilbert-sst-2" | ""
    >("");
    ...
    useEffect(() => {
        ...
        async function warmAndRefreshStatus() {
            try {
                // Warm both DistilBERT variants
                await warmModels(
                    ["distilbert", "distilbert-sst-2"],
                    apiKey || undefined,
                );
                const status = await modelStatus();
                if (!cancelled) setModelInfo(status);
            } catch {
        ...
        async function init() {
            try {
                ...
                if (!cancelled) setHealthInfo(h);
            } catch {
                if (!cancelled) setHealthInfo(null);
            }

            await warmAndRefreshStatus();

            // Keep models warm every 5 minutes as long as the UI tab is open
            intervalId = window.setInterval(
                () => {
                    void warmAndRefreshStatus();
                },
                5 * 60 * 1000,
            ); // 5 minutes
        }
    ...
    const [batchResult, setBatchResult] = useState<SentimentResponse[] | null>(
        null,
    );
        ...
        <div className="min-h-screen">
            ...
            <main className="mx-auto max-w-6xl px-4 py-6 grid gap-6 md:grid-cols-3">
                <section className="md:col-span-2 grid gap-6">
                    <div className="card p-5">
                        ...
                        <div className="grid gap-3">
                            ...
                            <textarea
                                className={`textarea min-h-[120px] ${overLimit ? "border-danger" : ""}`}
                                value={text}
                ...
                <span className={overLimit ? "text-danger" : "text-slate-400"}>
                  {overLimit
                      ? `Over limit by ${text.length - limit} characters`
                      : `${remaining} characters remaining (limit ${limit})`}
                </span>
                                <span className="text-slate-400">
                  Model: {model || healthInfo?.default_model || "vader"} •
                  Available:{" "}
                                    {healthInfo?.models_available?.join(", ") ||
                                        "vader, distilbert"}
                </span>
                            </div>
                            ...
                            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3 mt-2">
                                <div className="grid gap-1">
                                    ...
                                    >
                                        <option value="">
                                            Default ({healthInfo?.default_model || "vader"})
                                        </option>
                            ...
                            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3 mt-2">
                                ...
                                <div className="grid gap-1">
                                    ...
                                    <input
                                        ...
                                        value={limit}
                                        onChange={(e) =>
                                            setLimit(Number(e.target.value) || TEXT_LIMIT_DEFAULT)
                                        }
                                    />
                                </div>
                    ...
                    <div className="card p-5">
                        ...
                        <div className="grid gap-3">
                            ...
                            <div className="flex items-center justify-between text-xs text-slate-400">
                                ...
                                <span>
                  Progress: {batchProgress}/{batchTotal}
                </span>
                            </div>
                            ...
                            <div className="mt-5 grid gap-3">
                                ...
                                <div className="grid gap-3">
                                    {batchResult.map((r, i) => (
                                        <div
                                            key={i}
                                            className="p-3 rounded-lg border border-base-border bg-base-bg/70 flex items-center justify-between"
                                        >
                                            <div className="flex items-center gap-3">
                        <span
                            className={`px-2 py-0.5 rounded text-xs border ${r.sentiment === "positive" ? "border-success text-success" : r.sentiment === "negative" ? "border-danger text-danger" : "border-slate-500 text-slate-300"}`}
                        >
                          {r.sentiment}
                        </span>
                          ...
                          {(r.confidence * 100).toFixed(1)}%
                                            ...
                                            <div className="text-xs text-slate-400">
                                                {r.processing_time_ms} ms • {r.model}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                ...
                <aside className="grid gap-6">
                  ...
                  {healthInfo ? "OK" : "Unknown"}
                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-slate-400">Default model</span>
                                <span className="font-medium">
                  {healthInfo?.default_model || "vader"}
                </span>
                            </div>
                            <div className="flex items-center justify-between">
                  ...
                  {modelInfo?.default_model || "vader"}
                    ...
                    <div className="card p-5">
                        ...
                        <ul className="list-disc list-inside text-sm text-slate-300 space-y-1">
                            ...
                            <li>
                                Service scales to zero when idle, so the first request after a
                                pause may be slower.
                            </li>
                        </ul>
                    </div>
